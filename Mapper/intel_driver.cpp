...
bool intel_driver::GetNtGdiGetCOPPCompatibleOPMInformationInfo(HANDLE device_handle, uint64_t * out_kernel_function_ptr, uint8_t * out_kernel_original_bytes)
{
	...
	{
		std::string stringsysname;
		RTL_OSVERSIONINFOW osVersion = { 0 };
		osVersion.dwOSVersionInfoSize = sizeof(RTL_OSVERSIONINFOW);
		if (osVersion.dwBuildNumber >= 19033)
		{
			stringsysname = "win32kbase.sys";
		}
		else {
			stringsysname = "win32kfull.sys";
		}
		const uint64_t kernel_NtGdiGetCOPPCompatibleOPMInformation = GetKernelModuleExport(device_handle, utils::GetKernelModuleAddress(stringsysname), "NtGdiGetCOPPCompatibleOPMInformation");
